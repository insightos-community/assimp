# assimp: reproducible platform builds
...
Validated musl tag: [`musl-v6.0.4-2`](https://github.com/insightos-community/assimp/releases/tag/musl-v6.0.4-2);
source commit: `5c44654deee264983c1ffcc74bc47e251221fb29`. Use a normal clone so container packaging can read `.git`.

```bash
git clone https://github.com/insightos-community/assimp.git assimp-repro
cd assimp-repro
git checkout --detach musl-v6.0.4-2
test "$(git rev-parse HEAD)" = 5c44654deee264983c1ffcc74bc47e251221fb29
```

Native build prerequisites: C/C++ compiler, CMake, Ninja and zlib development headers. The native recipe may use bundled zlib; the musl release instead downloads a pinned zlib prefix through `ci/musl/fetch_dependencies.py`.
...
cmake -S . -B build-glibc -G Ninja \
  ...
  -DASSIMP_BUILD_TESTS=ON -DASSIMP_BUILD_ASSIMP_TOOLS=ON -DASSIMP_BUILD_SAMPLES=OFF -DASSIMP_WARNINGS_AS_ERRORS=OFF
cmake --build build-glibc --parallel 2
(cd . && ./build-glibc/bin/unit)
...
REPRO_WORK="$(mktemp -d "${TMPDIR:-/tmp}/assimp-musl.XXXXXXXX")"
...
- `assimp-6.0.4-musl-x86_64-prefix.tar.gz`
...
(cd . && ./build-macos/bin/unit)
cmake --install build-macos
```

The macOS lock uses `cmeel-assimp==6.0.5`; this musl fork releases Assimp 6.0.4. Do not substitute one ABI/version for the other silently.
...
```bash
gh workflow run musl-release.yml --repo insightos-community/assimp --ref musl-v6.0.4-2
gh run list --repo insightos-community/assimp --workflow musl-release.yml --limit 5
# Set REPRO_RUN_ID to the run ID printed above.
gh run watch "$REPRO_RUN_ID" --repo insightos-community/assimp --exit-status
gh run download "$REPRO_RUN_ID" --repo insightos-community/assimp --name musl-dist --dir downloaded-dist
```
